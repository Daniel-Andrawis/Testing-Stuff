...
def score_job(job, resume):
    ...
    skill_score = min(len(matched_skills) / max(len(skills) * 0.3, 1), 1.0) * 100
    ...
    keyword_score = min(len(matched_keywords) / max(len(exp_keywords) * 0.2, 1), 1.0) * 100
    ...
    cert_aliases = {
        ...
        "Tines Core Certification": ["tines", "soar"],
    }
    matched_certs = 0
    for cert in certs:
    ...
    org_score = 100 if org_match else 30  # still score non-target orgs, just lower
